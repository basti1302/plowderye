...
function getConversationByName(name) {
  ...
  for (var c in conversations) {
    if (conversations[c].name === name) {
      logger.debug('getConversationByName(%s): %j', name, conversations[c], {});
      return conversations[c];
    }
  }
  logger.debug('getConversationByName(%s): not found', name);
  return null;
};
...
function getConversationBySocketId(socketId) {
  logger.debug('getConversationBySocketId(%s): %j', socketId, conversationBySocket[socketId], {});
  return conversationBySocket[socketId];
};
