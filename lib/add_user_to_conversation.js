...
module.exports = function(request, conversation) {
  ...
  var data = userFilter.filter(request.user);
  data.conversation = conversation.id;
  logger.debug('%s broadcasting user-joined: %j', request.id, data, {});
  messageUtil.broadcast(request.socket, conversation, 'user-joined', data);
