...
function joinRoom(socket, room) {
  ...
  console.log('sending joinResult');
  ...
  storage.getMessagesStream(room).on('data', function(message) {
    ...
    console.log('sending users in room message');
    emitUsersInRoom(socket, room);
  });
}

function emitUsersInRoom(socket, room) {
  ...
  if (usersInRoom.length > 1) {
    ...
    usersInRoomSummary += '.';
    socket.emit('message', createSystemMessage(usersInRoomSummary));
  }
}
