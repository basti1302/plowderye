...
var nameAvailable = require('../name_available');
...
function processNameChange(request, name) {
  ...
  storage.fetchConversation(request.user.conversationId, function(err, broadcastConv) {
    if (err) { return logger.error(err); }
    messageUtil.broadcast(request.socket, broadcastConv, 'name-changed', broadcastObject);
  });
}
