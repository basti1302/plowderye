...
exports.onNewConnection = function(socket, conversation) {
  ...
  addUserToConversation(socket, { id: conversation.id });
...
function createConversation(socket, _conversation) {
  if (_conversation.name) {
    var conversation = convState.create(_conversation.name);
    io.sockets.emit('conversation-added', conversation);
    addUserToConversation(socket, conversation);
  } else {
    logger.warn('%s Client provided no conversation name when trying to create a new conversation. Ignoring create request. %j', socket.id, _conversation, {});
  }
}
...
function joinConversation(socket, _conversation) {
  ...
  if (conversation == null) {
    logger.warn('%s Can\'t find matching conversation for conversation id %s or conversation name %s. Ignoring join request.', socket.id, _conversation.id, _conversation.name);
    return;
  }
...
function addUserToConversation(socket, conversation) {
...
function handleCreateConversation(socket) {
  socket.on('create-conversation', function(conversation) {
    if (conversation) {
      leaveConversation(socket, convState.getConversationBySocket(socket));
      logger.debug('%s create %j', socket.id, conversation, {});
      createConversation(socket, conversation);
    }
  });
}

function handleJoinConversation(socket) {
